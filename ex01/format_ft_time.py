...
# It returns the current time in seconds since January 1,
#  1970 (called the Unix epoch).
# This value is a floating-point number,
#  useful for precise time calculations.
...
#  which allows embedding variables and expressions
# - directly inside the string using curly braces {}.
#  It's a simple way to format output.
# :, — tells Python to format the number with commas (e.g., 1,000,000)
# .4f — shows 4 digits after the decimal point
# .2e — formats in scientific notation with 2 digits after the decimal
print(
    f"Seconds since January 1, 1970: {epoch_time:,.4f} "
    f"or {epoch_time:.2e} in scientific notation"
)

# 'datetime.now()' gets the current local date and time with
